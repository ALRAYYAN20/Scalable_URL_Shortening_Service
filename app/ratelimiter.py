...
ip_limiter = Ratelimit( redis = redis_client,
                        limiter = FixedWindow ( max_request = 10, window = 60 )
                        ) # 60 secs

# user based limiter
user_limiter = Ratelimit( redis = redis_client,
                        limiter = FixedWindow ( max_request = 20, window = 60 )
                        ) # 60 secs
...
def check_user_rate_limit( request: Request):
    ...
    if not response.allowed:
        raise HTTPException ( status_code = 429, detail = " Too many request from this user, Try again later.")
